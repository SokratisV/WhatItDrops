...
Open an [issue](https://github.com/SokratisV/LootLink/issues/new/choose) using the **Bug
report** template. The most useful things to include:

- Your **WoW flavor/build** (TBC Classic / Anniversary) and **LootLink version**
...
- Whether it still happens with **only LootLink enabled** (rules out addon conflicts).
...
- The repo lives in a shared `Interface/AddOns` folder; only the `LootLink*` addons are
...
- The big **loot data files are generated**, not hand‑edited — see the regeneration
  scripts documented in [`LootLink/README.md`](LootLink/README.md)
  (`tools/*.ps1`). Don't edit `Data/LootLink*Items.lua` / `LootLinkBosses.lua` by hand.
